        ...
        for dtype in dtypes:
            ...
            for graph in graphs:
                ...
                for impl in implementations:
                    exp = filter(benchmarks, {  "graph": graph, 
                                                "direction": direction, 
                                                "name": impl,
                                                "irrep_dtype": dtype
                                            }, match_one=True)

                    data[direction][dtype][graph_lmap[graph]][labelmap[impl]] = np.mean(exp["benchmark"]["time_millis"])
    ...
    for i, direction in enumerate(directions):
        for j, dtype in enumerate(dtypes):
            for k, graph in enumerate(graphs):
                normalizing_value = data[direction][dtype][graph_lmap[graph]]["cuE-scattersum"] 
                for impl in implementations:
                    data[direction][dtype][graph_lmap[graph]][labelmap[impl]] = normalizing_value / data[direction][dtype][graph_lmap[graph]][labelmap[impl]]

            grouped_barchart(data[direction][dtype], axes[i][j], bar_height_fontsize=0, rotate_xlabels=True, colormap=colormap, hatchmap=hatchmap, group_spacing=6.0)
