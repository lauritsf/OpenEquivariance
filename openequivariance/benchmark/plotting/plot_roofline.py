...
def plot_roofline(data_folder):
    ...
    for i, desc in enumerate(configs):
        for direction in ["forward", "backward"]:
            ...
            for impl in implementations:
                exp = filter(benchmarks, {"config_label": desc, 
                                        "direction": direction, 
                                        "implementation_name": impl}, match_one=True)
                data[direction][desc][labelmap[impl]] = (exp["benchmark results"]["arithmetic_intensity (FLOPs / byte)"], np.mean(exp["benchmark results"]["throughputs_gflops"]))

    roofline_data = []
    marker_map = {"forward-cuE": "+", "backward-cuE": "X", "forward-ours": "P", "backward-ours": "X"}
    for i, desc in enumerate(configs):
        for direction in ["forward", "backward"]:
            ai, throughput = data[direction][desc][labelmap["LoopUnrollTP"]]
            label = f"{direction}-ours"
            roofline_data.append({"AI": float(ai), "throughput": throughput / 1000, "label": label, "marker": marker_map[label], "color": colormap['ours'], "markersize": 80})

            label = f"{direction}-cuE"
            ai, throughput = data[direction][desc][labelmap["CUETensorProduct"]]
            roofline_data.append({"AI": float(ai), "throughput": throughput / 1000, "label": label, "marker": marker_map[label], "color": colormap['cuE'], "markersize": 80})
    ...
    fig, ax = roofline_plot(draw_bounds, cpu_roofs, mem_bottlenecks, AI_v, roofline_data, fig_ratio=1.8, fig_dimension=4)
    ...
    for i, desc in enumerate(sorted(configs)):
        for direction in ["forward", "backward"]:
            for impl in implementations:
                ...
                long_desc = long_desc.replace("->", "$\\rightarrow$").replace(" x ", "$\ \\times\ $")
                ai_ours, throughput_ours = data[direction][desc][labelmap["LoopUnrollTP"]]
            ...
            result = ["\multirow{2}{*}{" + short_id + "}", "\multirow{2}{*}{" + long_desc + "}", dir_map[direction], f"{ai_ours:.1f}", throughput_cue, throughput_ours]
